...
If you have a local backend server running on localhost and need to connect it to the frontend similar to Create-React-App (https://create-react-app.dev/docs/proxying-api-requests-in-development), you can achieve it by following the below steps:

* npm install http-proxy-middleware --save
* Add the following to webpack.config.dev.js
...
```
  devServer: {
    ...
    proxy: {
      '/api': {
        target: 'http://localhost:8080', //Change the URL as per your local backend URL
        pathRewrite: {
          '^/api' : ''
        }
      }
...
* Now you can call your local API as '/api/URL' where URL is the backend endpoint. Please note that '/api/' is must.
